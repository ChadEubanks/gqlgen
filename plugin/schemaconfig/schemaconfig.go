...
func (m *Plugin) MutateConfig(cfg *config.Config) error {
	...
	cfg.Directives["bind"] = config.DirectiveConfig{
		SkipRuntime: true,
	}

	cfg.Directives["field"] = config.DirectiveConfig{
		SkipRuntime: true,
	}
	...
	for _, schemaType := range schema.Types {
		...
		if bd := schemaType.Directives.ForName("bind"); bd != nil {
		...
		if schemaType.Kind == ast.Object || schemaType.Kind == ast.InputObject {
			for _, field := range schemaType.Fields {
				if fd := field.Directives.ForName("field"); fd != nil {
					...
					if ra := fd.Arguments.ForName("resolver"); ra != nil {
